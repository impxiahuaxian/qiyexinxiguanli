...
/*
此日历控件兼容IE、FireFox ,界面清爽，纯脚本运行，实现了日期的回显功能，调用方便灵活 。

使用方法：
1、引用 <script src="jsdate.js" type="text/javascript"></script>
2、编写触发的脚本事件 this.Txt_Date.Attributes["onclick"] = "SelectDate(this,'yyyy-MM-dd')";
3、<input name="Txt_Date" type="text" maxlength="10" id="Txt_Date" onclick="SelectDate(this,'yyyy/MM/dd')" />
*/

var cal;
var isFocus=false; //是否为焦点
//以上为  2006-06-25 添加的变量
...
2006-06-25  修改为根据用户指定的 style 来确定；
*/
...
Calendar.prototype.draw = function() {
  ...
  var mvAry = [];
  //mvAry[mvAry.length]  = '  <form name="calendarForm" style="margin: 0px;">'; //因 <form> 不能嵌套， 2006-12-01 改用 Div
  ...
  tds[i].style.backgroundColor = calendar.colors["td_bg_out"];
    ...
    if (dateArray[i] != "&nbsp;"&&new Date().format(calendar.dateFormatStyle) >=
      ...
      if (new Date().format(calendar.dateFormatStyle) ==
        ...
        //continue; //若不想当天单元格的背景被下面的覆盖，请取消注释 →  2006-12-03 添加
      }//end if
      
      //设置已被选择的日期单元格背影色 2006-12-03 添加
...
Calendar.prototype.show = function (dateObj, popControl) {
  ...
  //this.date = new Date(dateObj.value.toDate(this.dateFormatStyle));//修改，带入用户指定的 style  
  this.date = (dateObj.value.length > 0) ? new Date(dateObj.value.toDate(this.dateFormatStyle)) : new Date() ;//2006-12-03 添加 → 若为空则显示当前月份
  this.year = this.date.getFullYear();
  this.month = this.date.getMonth();
  this.changeSelect();
  this.bindData();
...
//以下 2006-06-25 修改 → 用<iframe> 遮住 IE 的下拉框
/**//**//**//**//**//**//**//*
//设置控件显示或隐藏
...
document.write('</div>');
//var calendar = new Calendar();  //此句被 注释，否则 IE 将报错
